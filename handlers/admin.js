    ...
    try {
        ...
        if (ctx.message) {
            ...
            if ((messageText?.startsWith('/search') || step[chatId] === 'awaiting_search_keyword') && step[chatId] !== 'awaiting_note_keywords') {
                ...
                if (!keyword)
                    return ctx.reply('❗️Please provide a keyword.');
                const results = await searchNotesByKeyword(keyword);
                if (!results.length)
                    return ctx.reply('🔍 No notes found for this keyword.');
            ...
            if (step[chatId] === 'awaiting_category_name') {
                ...
                if (!categoryName)
                    return ctx.reply('Category name cannot be empty.');
            ...
            if (step[chatId] === 'awaiting_note_keywords') {
                ...
                if (!keywords?.length)
                    return ctx.reply('❗ Please enter at least one valid keyword.');
